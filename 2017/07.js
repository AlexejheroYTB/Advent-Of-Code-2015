...
myookpi (54)
olrgu (97)
swnafht (44) -> lbxdtms, jensj, zkpfzio, jobwpao, jxfimbl`.split("\n").map(p => {
    const [name, weight, _, ...children] = p.split(" ");
...
const imbalancedPrograms = [];

here: for (const program of programs) {
    if (!program.children || !program.children.length) continue;

    const weight = getWeight(program.children[0]);
    for (const child of program.children) {
        const childWeight = getWeight(child);
        if (childWeight != weight) {
            imbalancedPrograms.push(program.name);
            continue here;
        }
    }
}

for (const program of imbalancedPrograms) {
    if (getProgram(program).children.filter(e => imbalancedPrograms.includes(e)).length == 0) {
        console.log(program, getProgram(program).children);
        console.log(program, getProgram(program).children.map(n => getProgram(n).weight));
    }
}

function getWeight(name) {
    const thisProgram = getProgram(name);
    return thisProgram.weight + (thisProgram.children ?? []).reduce((acc, c) => acc + getWeight(c), 0);
}
