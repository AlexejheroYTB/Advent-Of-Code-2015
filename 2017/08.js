...
fw dec -971 if fz < 1922`.split("\n");

const registers = {};
let max = Number.MIN_SAFE_INTEGER;

for (const row of input) {
    const [register, action, amount, _, conditionRegister, condition, conditionAmount] = row.split(" ");

    if (!registers[register]) registers[register] = 0;
    if (!registers[conditionRegister]) registers[conditionRegister] = 0;

    if (eval(`${registers[conditionRegister]} ${condition} ${conditionAmount}`)) {
        eval(`registers["${register}"] ${action == "inc" ? "+=" : "-="} ${amount}`);
    }
