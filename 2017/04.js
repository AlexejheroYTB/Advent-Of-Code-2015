...
here: for (const row of input) {
    for (const word of row) {
        if (row.filter(w => w == word).length > 1) {
            continue here;
        }

        if (row.filter(w => getAnagrams(word).includes(w)).length > 1) {
            continue here;
        }
    }
    ok++;
...
function getAnagrams(word) {
    ...
    for (const permutation of permutations) {
        anagrams.push(permutation.join(""));
    }
    return anagrams;
}
