...
.split("\n").map(r => {
    r = r.split(/[pva]=</g).flatMap(m => m.split(/>/g)).flatMap(m => m.split(/, ?/g)).filter(e => e != "").map(Number);
    r[10] = 0;
    return r;
});
...
for (let t = 0; t < 100000; t++) {
    for (let i = 0; i < input.length; i++) {
        ...
        p[10] += Math.abs(p[0]) + Math.abs(p[1]) + Math.abs(p[2]);
    }

    const grouped = groupBy(input, p => `${p[0]},${p[1]},${p[2]}`);
