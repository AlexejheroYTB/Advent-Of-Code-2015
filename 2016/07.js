// My first attempt was using RegEx, which only worked for the 1st part of the challenge. For the 2nd part, I kept getting incorrect results so I switched to JS.
// Honestly, I should have used JS in the first place...
//
...
tivudfusgnewzshs[mausfjbgxmyibin]yponuityptavbhekrlg[qeyafuevtlqemtfa]owtdxadrwwbxbrkl[obfcyxbifipwhduubu]mjocivgvrcbrllso`.split(/[\n\r]/g);

let result = 0;
here: for (const ip of input) {
  const outside = ip.replace(/\[.*?\]/g, "[]");

  for (const char1 of "qwertyuiopasdfghjklzxcvbnm".split("")) {
    for (const char2 of "qwertyuiopasdfghjklzxcvbnm".split("")) {
      if (char1 == char2) continue;

      if (ip.match(new RegExp(`\\[[a-z]*?${char1}${char2}${char1}[a-z]*?\\]`, "g")) && outside.includes(`${char2}${char1}${char2}`)) {
        result++;
        console.log(ip);
        continue here;
      }
