...
for (const i of input) {
  if (i.startsWith("rect")) {
    const width = i.substring(5).split("x")[0];
    const height = i.substring(5).split("x")[1];
    ...
    const y = i.substring(13).split(" by ")[0];
    const shift = i.substring(13).split(" by ")[1];
    const row = screen[y];
    screen[y] = row.slice(WIDTH - shift).concat(row.slice(0, WIDTH - shift));
  } else if (i.startsWith("rotate column")) {
    const x = i.substring(16).split(" by ")[0];
    const shift = i.substring(16).split(" by ")[1];
    const column = screen.map(row => row[x]);
    screen.forEach((row, i) => {
      let place = (i - shift);
      if (place < 0) place = HEIGHT + place;
      row[x] = column[place % HEIGHT];
    });
  }
}
