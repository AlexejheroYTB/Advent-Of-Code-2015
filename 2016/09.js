...
function analyzeText(text)
{
    ...
    let instructionType = 0, letterNum = 0, repeatAmont = 0;

    for (const char of text.split(""))
    {
        switch (instructionType)
        {
            case 0: // Ready to read whatever
            {
                switch (char) 
                {
                    case "(":
                        instructionType = 1; // Should read letter amount
                        continue;

                    default:
                        if (result[result.length - 1].type != "text") result.push({type:"text", value:""});
                        result[result.length-1].value += char;
                        continue;
                }
            }
            case 1: // Ready to read letter amount
            {
                switch (char)
                {
                    case "x":
                        instructionType = 2; // Should read repeat
                        continue;

                    default:
                        letterNum = letterNum * 10 + parseInt(char, 10);
                        continue;
                }
            }
            case 2: // Ready to read repeat amount
            {
                switch (char)
                {
                    case ")":
                        instructionType = 3; // Read stuff to repeat
                        continue;

                    default:
                        repeatAmont = repeatAmont * 10 + parseInt(char, 10);
                        continue;
                }
            }
            case 3: // Ready to read stuff to repeat
            {
                if (result[result.length - 1].type != "repeat") result.push({type:"repeat", value:"", times:0});
                ...
                if (letterNum == 0) 
                {
                    result[result.length - 1].times = repeatAmont;
                    repeatAmont = 0;
                    result.push({type:"text", value:""});
                    instructionType = 0; // Exit instruction
                }
...
function parseData(data) {
    for (const d of data) 
    {
        if (d.type == "repeat" && d.value.includes("(") && !d.parsedValue) {
            d.parsedValue = analyzeText(d.value);
        }

        if (d.parsedValue) parseData(d.parsedValue);
    }
    return data;
}
...
function getLength(data) {
    ...
    for (const d of data) 
    {
        if (d.type == "text") length += d.value.length;
        else if (d.type == "repeat" && !d.parsedValue) length += d.value.length * d.times;
        else if (d.type == "repeat" && d.parsedValue) length += getLength(d.parsedValue) * d.times;
    }
...
// Part 1
// console.log(getLength(analyzeText(input)))

console.log(getLength(parseData(analyzeText(input))))
