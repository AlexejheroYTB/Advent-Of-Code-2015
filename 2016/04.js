...
for (var room of input) {
  var name = room.substr(0, room.length - 11).split("")/*.filter(c => c != "-")*/.join("");
  var sector = parseInt(room.substr(room.length - 10).substr(0, 3));
  // var check = room.substr(room.length - 6).substr(0, 5);

  // if (getcheck(name) == check) sum += sector;

  var parsed = parseName(name, sector);

  if (parsed.includes("north"))

    console.log(sector);
}

function parseName(name, sector) {
  var newName = "";
  for (var char of name.split("")) {
    if (char == "-") {
      newName += " ";
      continue;
    }
    newName += addToChar(char, sector);
  }
  return newName;
}

function addToChar(char, amount) {
