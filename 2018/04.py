data = """[1518-05-30 00:27] wakes up
...
[1518-04-03 00:48] wakes up""".splitlines()
data.sort()

guards = [[0 for _ in range(60)] for _ in range(10000)]
curr_guard = -1
fell_asleep_at = -1
...
for area in data:
    ...
    time = int(splits[1][3:-1])
    action = splits[2]

    match action:
        case "Guard":
            curr_guard = int(splits[3][1:])

        case "falls":
            fell_asleep_at = time

        case "wakes":
            for i in range(fell_asleep_at, time):
...
for i in range(10000):
    if sum(guards[i]) > max_guard_sleep:
        max_guard_num = i
        max_guard_sleep = sum(guards[i])

max_minute_num = -1
max_minute_sleep = -1
for i in range(60):
    if guards[max_guard_num][i] > max_minute_sleep:
        max_minute_num = i
        max_minute_sleep = guards[max_guard_num][i]

print("Part 1:", max_guard_num * max_minute_num)
...
for g in range(10000):
    for m in range(60):
        if guards[g][m] > max_sleep:
            max_guard_num = g
            max_minute_num = m
            max_sleep = guards[g][m]

print("Part 2:", max_guard_num * max_minute_num)
